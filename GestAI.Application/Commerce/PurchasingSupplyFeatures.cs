    ...
    : IRequestHandler<GetSupplierAccountsQuery, AppResult<List<SupplierAccountListItemDto>>>
{
    public async Task<AppResult<List<SupplierAccountListItemDto>>> Handle(GetSupplierAccountsQuery request, CancellationToken ct)
    {
        ...
        var query = suppliers.Select(x => new SupplierAccountListItemDto(
            x.Id,
            x.Name,
            x.TaxId,
            x.IsActive,
            (db.SupplierAccountMovements.Where(m => m.AccountId == scope.AccountId && m.SupplierId == x.Id).Sum(m => (decimal?)m.DebitAmount) ?? 0m) - (db.SupplierAccountMovements.Where(m => m.AccountId == scope.AccountId && m.SupplierId == x.Id).Sum(m => (decimal?)m.CreditAmount) ?? 0m),
            db.SupplierAccountMovements.Where(m => m.AccountId == scope.AccountId && m.SupplierId == x.Id).Sum(m => (decimal?)m.DebitAmount) ?? 0m,
            db.SupplierAccountMovements.Where(m => m.AccountId == scope.AccountId && m.SupplierId == x.Id).Sum(m => (decimal?)m.CreditAmount) ?? 0m,
            db.PurchaseDocuments.Count(p => p.AccountId == scope.AccountId && p.SupplierId == x.Id),
            db.SupplierAccountMovements.Where(m => m.AccountId == scope.AccountId && m.SupplierId == x.Id).Max(m => (DateTime?)m.IssuedAtUtc)));

        if (request.OnlyWithBalance == true)
            query = query.Where(x => x.Balance != 0m);

        return AppResult<List<SupplierAccountListItemDto>>.Ok(await query.OrderByDescending(x => x.Balance).ThenBy(x => x.SupplierName).ToListAsync(ct));
    }
}
